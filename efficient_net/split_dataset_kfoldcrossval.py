...
def get_kfold_directory_list(
    ...
    Returns:
        dict[str,list[str]]: Returns a dictionary of two lists containing the folder paths.
    """
    # First, save all directories and their corresponding fold indices:
    list_kfold_dirs = []
    for dir, _, _ in os.walk(path_image_dir):
        ...
        if folder_name.startswith("fold_"):
            list_kfold_dirs.append(
                {
                    "idx_fold": int(folder_name.split("_")[-1]),
                    "train": dir + "/train",
                    "val": dir + "/val",
                }
            )
    ...
    """
    Then, make two separate lists and save the folders in an ORDERED manner.
    The order is important as the training loop should iterate from the 
    smallest to the largest fold index.
    """
    list_paths_fold_train_dirs = [None] * len(list_kfold_dirs)
    list_paths_fold_val_dirs = [None] * len(list_kfold_dirs)

    for dict_path_dir in list_kfold_dirs:
    ...
    return {
        "list_paths_fold_train_dirs": list_paths_fold_train_dirs,
        "list_paths_fold_val_dirs": list_paths_fold_val_dirs,
    }


if __name__ == "__main__":
    path_source_data = "/home/matthias/workspace/Coding/00_vista_medizina/00_data/2024-11-08/kaggle_bone_fracture_detection_small_2024-11-08/bf_orig_bgr_classification_7cl_kfold/all"
    path_dest_data = "/home/matthias/workspace/Coding/00_vista_medizina/00_data/2024-11-08/kaggle_bone_fracture_detection_small_2024-11-08/bf_orig_bgr_classification_7cl_kfold/w_folds"
